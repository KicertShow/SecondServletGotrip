...
<body>
    ...
    <script>
		...
		Boolean isLoggedIntoComment = (Boolean) request.getAttribute("isLoggedIntoComment"); 
		%>
			try {
		        if (<%=isLoggedIn%> == false) {
		        	document.getElementById("login-status").textContent = '帳號不存在或密碼輸入錯誤';
		        }
		        if (<%=isLoggedIntoComment%> == false) {
		        	document.getElementById("login-status").textContent = '請先登入再使用後台系統';
		        }
			} catch {
